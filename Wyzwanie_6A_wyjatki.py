...
## WYZWANIE 6A - wyjątki w poprzednich zadaniach
...
class Ulamek:
    ...
    def spr_mianownik(self):
        if self.mianownik == 0:
            raise ZeroDivisionError("\t dzielenie przez zero")
        else:
            self.ulamek = round(self.licznik / self.mianownik,  5)

    def spr_mianownik_t(self):
        try:
            self.spr_mianownik()
        except ZeroDivisionError as error:
            print("Error - szczegóły poniżej:")
            print(error)
    ...
    def is_number(self):
        try:
            ...
            print("Error - licznik  nie jest liczbą")
